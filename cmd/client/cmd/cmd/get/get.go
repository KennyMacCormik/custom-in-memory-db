...
func Init(cfg *conf.Config) *cobra.Command {
	var cmd = &cobra.Command{
		Use:   "get <key>",
		Short: "Get <value> associated with <key>",
		Long:  fmt.Sprintf("Executes get command. Reads <key> and prints it's value\n<key> expected to match %s pattern", validatorTag),
		Args:  args,
		Run:   run,
	}
...
func run(cmd *cobra.Command, args []string) {
	...
	fmt.Println(string(shared.InvokeTcp(cfg, strings.Join([]string{"GET ", args[0], "\n"}, ""))))
}
